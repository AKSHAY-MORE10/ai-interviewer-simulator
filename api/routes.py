...
def _bootstrap_session(session_id: str):
    state = interview_graph.get_state(_config(session_id))
    if state and state.values and state.values.get("question"):
        return state
    ...
    return interview_graph.get_state(_config(session_id))
...
def submit(req: SubmitRequest):
    ...
    # Bootstrap a fresh session if the caller submits without starting first.
    state = _bootstrap_session(req.session_id)
    current_question   = ""
    current_round      = 1
